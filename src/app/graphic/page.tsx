...
function GoodGraphicDesignSection({ isDark }: { isDark: boolean }) {
  ...
  return (
    <section className="graphic-design-elements pb-12 pt-4 sm:pb-20 sm:pt-6">
      <div className="mx-auto px-3 sm:px-5 lg:px-8">
        ...
        <p
          className="mt-6  text-[11px] uppercase leading-[1.85] tracking-[0.05em] sm:mt-8 sm:text-xs sm:leading-[1.9]"
          style={{ color: mutedText }}
        >
          {DESIGN_ELEMENTS_INTRO}
        </p>
...
function CreativeServicesSection({
  ...
  const renderDesktopRow = (row: 0 | 1) => {
    const rowServices = CREATIVE_SERVICES.filter((service) => service.row === row);
